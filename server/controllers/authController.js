...
const refresh = async (req, res) => {
    ...
    if (!token) {
        return res.status(401).json({ success: false, message: "Please provide a refresh token" });
    }
...
const switchProfile = async (req, res) => {
    ...
    try {
        ...
        const refreshToken = generateRefreshToken(data);
        return res.status(200).json({
            success: true, message: `${user.accountType}`,
            accessToken, refreshToken, role: user.accountType, author: user.username
        });
    } catch (error) {
